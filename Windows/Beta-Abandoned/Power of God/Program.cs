    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                {
                    if (File.Exists("power.of.god/settings.json")) Settings.LoadFromJson();
                    else Settings.LoadDefault();
                    if (Settings.UserSettings.UpdatesEnabled)
                    {
                        var choice =
                            MessageBox.Show(
                ...
                if (safeToRun)
                {
                    var l = PreRelease.GetPreReleaseParts("b1.15a001");
                    foreach (var i in l)
                    {
                        MessageBox.Show(i);
                    }
                    if (!Network.IsPreRelease) Application.Run(new Splash());
